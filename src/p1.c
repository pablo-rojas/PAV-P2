...
int main(int argc, char *argv[])
{
    ...
    if (bps != 16)
    {
        printf("Error con el formato del audio. El numero de bits por muestra debe de ser 16.\n");
        return -1;
    }

    N = durTrm * fm;
    if ((buffer = malloc(N * numChannels * bps / 8)) == 0 ||
        (x = malloc(N * sizeof(*x))) == 0 || (x2 = malloc(2 * N * sizeof(*x))) == 0)
    {
    ...
    while (lee_wave(buffer, bps / 8, (N * numChannels), fpWave) == N * numChannels)
    {
        for (int n = 0; n < N; n++)
        {
            x2[n + N] = x[n];
        ...
        if (argc == 2)
        {
            printf("%d\t%f\t%f\t%f\t%f\n", trm, compute_power(x, N), compute_power_hamming(x2, N), compute_am(x, N), compute_zcr(x, N, fm));
        }
        else if (argc == 3)
        {
